...
import { clearNft } from '../../store/actions';
...
const Resell = ({nftId}) => {
  ...
  const nftDetailState = useSelector(selectors.nftDetailState);
	const consideringNFT = nftDetailState.data ? nftDetailState.data : null;
  ...
  if(consideringNFT) {
    ...
    if (isScreenMounted.current && consideringNFT.status !== method && method === '') {
  ...
  const handleValidation = () => {
    ...
    if (method === 'buy_now' && (!fields["price"] || fields["price"] <= 0)) {
      formIsValid = false;
      setErrors({...errors, price : "This field is required."});
    }

    if (method !== 'buy_now' && (!fields["priceover"] || fields["priceover"] <= 0)) {
      formIsValid = false;
      setErrors({...errors, priceover : "This field is required."});
    }

    if (method === 'on_auction' && auctionInterval <= 0) {
      formIsValid = false;
      setErrors({...errors, deadline : "This field is required."});
    }

    if (method === 'has_offers' && (!fields["max_bid"] || fields["max_bid"] <= 0)) {
      formIsValid = false;
      setErrors({...errors, max_bid : "This field is required."});
    }
                      ...
                      <div className="row">
                        <div className="col-md-8">                          
                        {
                          ...
                          <input type="number" name="priceover" id="priceover" 
                            ...
                            className="form-control mb-2" placeholder="enter start price" step="0.001"/>
                          :
                          <input type="number" name="priceover" id="priceover" 
                            onChange={(e) => handleChange(e)} 
                            className="form-control mb-2" placeholder="enter start price" step="0.001"/>
            ...
            <div className="nft__item m-0">
              {method === "on_auction" && deadline && (
                <div className="de_countdown">
                  <Clock deadline={auctionInterval} checkAuctionStatus={checkAuctionStatus} />
                </div>
              )}
