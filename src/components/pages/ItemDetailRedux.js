...
const ItemDetailRedux = ({ nftId }) => {
	...
	const handleBuynowBtnClick = async () => {
		const isCheck = await checkNetwork();
    	if (!isCheck) return;
	...
	const handleCancelBuynowBtnClick = async () => {
    	...
    	if (!isCheck) return;
		setLoading(true);	
		let result = await destroySale(nft);
		if (result.success) {
		...
		await Axios.put(`${api.baseUrl}${api.nfts}/${nft.id}`, {"deadline" : null, "situation":"saled" }, {}
		).catch(err => {
	...
	const handlerOnCheckoutBuynow = async (nft) => {
		...
		if (balanceOfCustomer < nft.price) {
			...
			if(nft.bids[0] !== undefined)  {
				result = await placeBid(curUser.author, nft, nft.bids[0].value);	
			} else {
				result = await placeBid(curUser.author, nft, nft.price);	
			}
			if (!result.success) {
				NotificationManager.error(result.status, "Error");
				setLoading(false);
	...
	const handlerOnCheckoutBid = async (nft) => {
		...
		var biddedPrice = bidPrice;
		if (!biddedPrice) {
			NotificationManager.error("Invalid Bid Price. Please check it again.", "Error");
			return;
		}
		...
		else if (nft.author.id === curUser.author.id) 
		{
			...
			if(nft && nft.status && nft.status === "has_offers")
			{
				...
				if((nft.max_bid && nft.bids.length && nft.bids.length<nft.max_bid) || !nft.bids.length )
				{
					...
					setLoading(true);
					let result;
					result = await placeBid(curUser.author, nft, biddedPrice);	
					if (result.success) {							
								...
								<div className="de_tab_content">
												...
												nft.bids && nft.bids.map((bid, index) => (
													<div className="p_list" key={index}>
														...
														<div className="p_list_info">
															Bid {bid && bid.author && bid.author.id && nft && nft.author && bid.author.id === nft.author.id && 'accepted'} <b>{bid.value} {nft.token_type}</b>
									...
									{openMenu1 && (
										<div className="tab-2 onStep fadeIn">
											{nft.history && nft.history.map((item, index) => (
												<div className="p_list" key={index}>
													<div className="p_list_pp">
														<span>
															<img className="lazy" src={item && item.to && item.to.avatar ? api.baseUrl + item.to.avatar.url : `${window.location.origin}/img/avatar.jpg`} alt="" />
															<i className="fa fa-check"></i>
														</span>
													</div>
													<div className="p_list_info">
														Bought <b>{item.price} {item.token_type}</b>
														<span>by <b>{item.to && item.to.username && item.to.username}</b> at {moment(item.created_at).format('L, LT')}</span>
													</div>
												</div>
											))}
										</div>
					...
					<div className='maincheckout'>
						...
						<div className='detailcheckout mt-4'>
							<div className='listcheckout'>
								...
								<input type="number" id="bidPrice" min={highestPastBidValue} step="0.001" defaultValue={highestPastBidValue} className="form-control" onChange={(e) => handleBidChange(e)}/>
							</div>
						</div>
						<div className='heading mt-3'>
							<p>Your balance</p>
